...
func (a *Adapter) RegisterRoutes(kuta *kuta.Kuta) error {
	...
	// Public routes
	api.Post("/sign-up", a.signUp(kuta))
	api.Post("/sign-in", a.signIn(kuta))

	// Protected routes
	api.Post("/sign-out", a.requireAuth(kuta), a.signOut(kuta))
	api.Get("/session", a.requireAuth(kuta), a.session(kuta))
